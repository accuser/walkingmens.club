/**
 * DNS configuration guide API endpoint
 * GET /api/admin/subdomains/[hostname]/dns-guide - Get DNS configuration instructions
 */
...
export const GET: RequestHandler = async (event) => {
	try {
		...
		const guide = validationService.generateDNSConfigurationGuide(hostname);
		...
		console.error('DNS guide API error:', error);
		return json({ error: 'Failed to generate DNS guide' }, { status: 500 });
	}
};