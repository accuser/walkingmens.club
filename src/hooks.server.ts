...
export const handle: Handle = async ({ event, resolve }) => {
	...
	if (event.platform) {
		try {
			const healthStatus = await getServiceHealthStatus();
			response.headers.set('X-System-Database', healthStatus.database ? 'available' : 'unavailable');
			response.headers.set('X-System-Fallback', healthStatus.fallback ? 'available' : 'unavailable');
